...
%zy ma f el describtion bs 7bet a7ot comment zy e5watha ^_^
grid_build(N,M):-
...
% check when you enter gridGen(3,M).

%grid_gen(_,[]).
%grid_gen(N,[H|T]):-checkSmallerN(H,N),grid_gen(N,T).

grid_gen(N,M):- gridGenCall(N,M).
...
%check if it takes empty list  will it either return true or false

%check_num_grid([]).
%check_num_grid([H|T]):-checkNumList(H),check_num_grid(T).
...
gridGenCall(N,M):- grid_build(N,M),getAllPossibleList(N,L),gridGen2(M,L).
...
% helsinki(N,M):- grid_gen(N,M),distinct_rows(M),check_num_grid(M),row_col_match(M).
